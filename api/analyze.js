...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({coinprint_version:'13.2.0',status:verified?'VERIFIED':'BLOCKED',valuation_allowed:verified,identity:verified?{...supported[0],catalogue_id:catalogue.record.catalogue_id}:null,confidence:verified?Math.min(clamp(supported[0].score),catalogue.record.score):0,blockers:[...new Set(blockers)],quality:photoQuality,features,measurements,physical_gate:physicalGate,catalogue:{matched:catalogueMatches.map(x=>({catalogue_id:x.catalogue_id,label:x.label,score:x.score,contradictions:x.contradictions})),gate:catalogue.reason},candidate_verdicts:verifier.candidate_verdicts||[],next_needed:[...new Set([...(a.missing_views||[]),...(b.missing_views||[]),...(verifier.missing_views||[])])].filter(Boolean),audit:{passes:3,independent_primary_passes:2,catalogue_schema:'1.0',timestamp:new Date().toISOString()}});
